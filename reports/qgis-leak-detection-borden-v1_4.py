"""
QGIS Python Console Script — Vineyard Leak Detection v1.4 (Borden preset)

Raster-first leak candidate workflow using:
- full thermal ortho
- vine mask raster
- vine-only thermal raster
- non-vine thermal raster

This Windows/QGIS-safe version avoids temp-file deletion during active processing.
Copy/paste this whole file into the QGIS Python editor and run it as-is.
"""
...
THERMAL_RASTER = r"H:\2025_05_25_Borden_thermal\leak-detection\full_thermal.tif"
VINE_MASK_RASTER = r"H:\2025_05_25_Borden_thermal\leak-detection\vine_mask.tif"
VINE_THERMAL_RASTER = r"H:\2025_05_25_Borden_thermal\leak-detection\vine_only_thermal.tif"
NONVINE_THERMAL_RASTER = r"H:\2025_05_25_Borden_thermal\leak-detection\nonvine_thermal.tif"

OUTPUT_DIR = r"H:\2025_05_25_Borden_thermal\new_leak_detection_outputs"
...
def safe_remove_dataset(path_str):
    ...
    if p.exists() and p.is_file():
        try:
            ...
            # Windows/QGIS sometimes keeps handles open briefly.
            # Skip deletion instead of crashing.
            pass


def tmp_gpkg_path(out_path, suffix):
    p = Path(out_path)
    return str(p.with_name(f"{p.stem}_{suffix}.gpkg"))
...
def raster_array_values(raster_path):
    if gdal is None:
        raise RuntimeError("GDAL Python bindings are not available in this QGIS environment.")
...
def threshold_value(method, stats):
    if method == "q20":
        return stats["q20"]
    if method == "mean_minus_1sd":
        return stats["mean"] - stats["stddev"]
    if method == "mean_minus_2sd":
...
def filter_value_one_and_add_area(input_vector, out_path):
    tmp_path = tmp_gpkg_path(out_path, "tmp")

    safe_remove_dataset(tmp_path)
    safe_remove_dataset(out_path)
...
def add_metadata_fields(input_vector, source_name, method, stats, out_path):
    ...
    provider.addAttributes([QgsField(name, qtype) for name, qtype, _ in fields_to_add])
    layer.updateFields()

    idx = {f.name(): i for i, f in enumerate(layer.fields())}
    layer.startEditing()
    for feat in layer.getFeatures():
        for name, _qtype, value in fields_to_add:
            feat[idx[name]] = value
...
def main():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Leak Detection v1.4 Stats Report\n")
        f.write("=" * 40 + "\n\n")
