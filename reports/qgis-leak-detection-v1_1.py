"""
QGIS Python Console Script — Vineyard Leak Detection v1.1

Purpose
-------
Leak detection from existing thermal products you already have:
- full thermal ortho
- vine mask raster (optional in this version, mostly for QA / future use)
- vine-only thermal raster
- non-vine thermal raster

This version improves on v1 by:
- fixing the temporary output path bug
- avoiding raster-to-point conversion for quantile estimation when numpy is available
- using GDAL-backed raster reads directly when possible
- falling back cleanly when needed

What it does
------------
For each source raster (whole / vine / nonvine), it:
1. computes raster statistics
2. computes thresholds:
   - lower quintile (q20)
   - mean - 1 stddev
   - mean - 2 stddev
3. creates binary cool-anomaly rasters
4. removes tiny noise patches with sieve
5. polygonizes anomaly zones
6. filters tiny polygons
7. adds metadata + simple confidence score
8. merges all candidate layers into one ranked output

How to use
----------
1. Open QGIS.
2. Open the Python Console editor.
3. Load this script.
4. Edit the INPUTS section.
5. Run.

Outputs
-------
- per-source / per-threshold anomaly rasters
- per-source / per-threshold candidate polygons
- merged ranked candidate polygons
- text stats report

Notes
-----
- v1.1 is still raster-first, not row-section based.
- Best used as a first-pass inspection candidate workflow.
- Lower quintile is retained because the paper found it most effective.
"""
...
# Optional GDAL direct raster read
try:
...
PRIMARY_METHOD = "q20"  # q20 | mean_minus_1sd | mean_minus_2sd
...
def raster_array_values(raster_path):
    """
    Return finite raster values as a 1D array/list.
    Prefer GDAL + numpy. If unavailable, fail clearly.
    """
    if gdal is None:
        raise RuntimeError(
            "GDAL Python bindings are not available in this QGIS environment. "
            "This script expects osgeo.gdal to be present."
        )
    ...
    if np is None:
        # fallback to pure Python list handling if numpy import failed
        values = []
        rows, cols = arr.shape
        for r in range(rows):
            for c in range(cols):
...
with open(report_path, "w", encoding="utf-8") as f:
    f.write("Leak Detection v1.1 Stats Report\n")
    f.write("=" * 40 + "\n\n")
